...
type View = 'feed' | 'post' | 'profile';
  ...
  // Fetch real data from Firestore on initial load
  useEffect(() => {
    // Función asíncrona para cargar los posts
    const fetchPosts = async () => {
      try {
        // 1. Acceder a la colección 'posts' que creaste en Firebase
        const postsCollection = collection(db, 'posts');
        
        // 2. Ejecutar la consulta y obtener los documentos (posts)
        const postsSnapshot = await getDocs(postsCollection);
        
        // 3. Transformar los documentos de Firebase al formato Post[]
        const fetchedPosts: Post[] = postsSnapshot.docs.map(doc => ({
          id: doc.id,
          // Es crucial usar doc.data() y asegurar que el tipo sea correcto
