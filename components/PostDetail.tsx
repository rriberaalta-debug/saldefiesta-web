...
import { X, Heart, MessageCircle, Send, ShieldOff, Share2 } from 'lucide-react';
...
const PostDetail: React.FC<PostDetailProps> = ({ post, user, comments, users, onClose, onLike, onAddComment, onUserSelect, currentUser, onBlockUser }) => {
            ...
            <div className="bg-black/50 backdrop-blur-lg p-4 sm:p-6 rounded-2xl max-w-4xl mx-auto animate-fade-in">
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="lg:w-1/2 flex flex-col">
                        <div className="flex items-center mb-4 cursor-pointer" onClick={() => onUserSelect(user.id)}>
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-300">{format(new Date(post.timestamp), "d 'de' MMMM, yyyy 'a las' H:mm", { locale: es })}</p>
                            </div>
                        </div>
                        <div className="flex-grow overflow-y-auto pr-2" style={{ maxHeight: 'calc(100vh - 400px)' }}>
                            ...
                            <div className="flex items-center space-x-6 mb-4 text-gray-300">
                               <button onClick={() => onLike(post.id)} className={`flex items-center space-x-2 hover:text-white transition-colors ${post.liked ? 'text-red-500' : ''}`}>
                                 <Heart fill={post.liked ? 'currentColor' : 'none'} size={24} />
